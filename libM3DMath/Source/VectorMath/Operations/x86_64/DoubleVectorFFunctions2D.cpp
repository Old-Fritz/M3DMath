...
DoubleVectorF VECCALL M3DM::doubleVecFCross2D(DoubleVectorF vec1, DoubleVectorF vec2)
{
	__m256 result = _mm256_sub_ps(
		_mm256_mul_ps(vec1, _mm256_shuffle_ps(vec2, vec2, _MM_SHUFFLE(3, 2, 1, 1))),
		_mm256_mul_ps(vec2, _mm256_shuffle_ps(vec1, vec1, _MM_SHUFFLE(3, 2, 1, 1)))
	);
...
{
	return _mm256_dp_ps(vec1, vec2, 0b00111111);
}
